    ...
    <div>
      ...
      <div className="my-16 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-[2px]">
        {projects.map((project: ProjectType, i: number) => (
          <Link className="relative col-span-1 overflow-hidden group after:absolute after:inset-0 after:bg-gradient-to-tr after:from-black/20" href={`/projects/${project.slug}/`} key={i}>
            <Image
              src={urlFor(project.image).width(1000).height(648).fit("crop").url()}
              alt={project.name}
              width={1000}
              height={648}
              className="group-hover:scale-110 transition duration-700 ease-out grayscale group-hover:grayscale-0"
            />
            <h2 className="absolute bottom-0 left-0 text-sm p-2 text-cream/80 group-hover:text-cream transition duration-500 leading-tight z-20">{project.name}</h2>
          </Link>
        ))}
      </div>
    </div>
