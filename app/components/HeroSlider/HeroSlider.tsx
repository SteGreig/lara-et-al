  ...
  return (
    <div className="hero-slider container h-[80vh] max-h-[1100px] xl:h-auto flex">
      <div className="relative h-auto w-full">
        ...
        {heroHeadline && (
          <div className="absolute bottom-0 left-0 z-10 ~p-6/20 w-full md:w-3/4 lg:w-2/3 xl:w-1/2 text-cream">
            <p className="~text-3xl/7xl leading-none text-balance">{heroHeadline}</p>
        ...
        {slides.map((slide, i: number) => {
          const SlideEl = ({
            ...
            slide.projectSlug ? (
              <Link className={className} href={`/projects/${slide.projectSlug}/`}>{children}</Link>
            ) : (
